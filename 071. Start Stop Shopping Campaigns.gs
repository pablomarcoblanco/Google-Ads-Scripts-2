...
*
* Goal: Starts and stops Campaigns
...
function enable_pause(totalDays, hour){
  ...
  var labelA = campaignLabelA;
  var labelB = campaignLabelB;
...
function objectIterator(label){
  ...
  var iterator = "AdsApp.​shoppingCampaigns()";
...
function PauseCampaigns(label){
  ...
  if (!iterator.hasNext()) {
    Logger.log("Warning: no Campaign found with the label '" + label + "'. No Campaign paused.");
  }
...
function EnableCampaigns(label){
  var iterator = eval(objectIterator(label));
  if (!iterator.hasNext()) {
    Logger.log("Warning: no Campaign found with the label '" + label + "'. No Campaign enabled.");
  }
  while(iterator.hasNext()){
    var object = iterator.next();
    object.enable();
