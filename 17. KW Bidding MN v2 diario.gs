...
var kwIncomePerLead = 23.8;
// This is the estimated income per Lead we are getting for the Campaign Marca_Seguros_Mobile
...
function main() {
  ...
  while(rows.hasNext()){
    ...
    // Deal with the problems of SearchImpressionShare
    if (row['SearchImpressionShare'] == "--") {row['SearchImpressionShare'] = "0.00"};
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
  ...
  for(var uId in kwData) {
    ...
    // First check that periodA and periodB have both enough clicks
    if (kwData[uId]['perTA']['DayClicks'] >=  minClicks &&
      ...
      // If perTA has enough clicks and perTB is 0, this means that this is the
      ...
      // witht the same CPCmax. We could not compare
      else if (kwData[uId]['perTA']['DayClicks'] >=  minClicks &&
               kwData[uId]['perTB']['DayClicks'] ==  -100) {
      ...
      Logger.log("The Keyword: " + uId + " has reached enough clicks in TA. " +
        ...
        // If we do not reach enough clicks in both periods, we will keep the
        // CPCmax as it is now
  ...
  // COMPARED EMNS, WE NEED TO CHANGE THE PERIODS
...
// Return EMNNorm
  ...
  // Calculate the EMN
  var EMN = kwDataUId[per]['DayClicks'] * convRate * kwIncomePerLead - kwDataUId[per]['DayCGAds'];
  ...
  Logger.log("KWIncomePerLead: " + kwIncomePerLead);
...
// RETURN: Floating point value with the new CPCMax
  ...
  var slope = (kwDataUI['perTB']['EMNNorm'] - kwDataUI['perTA']['EMNNorm']) /
               (kwDataUI['perTB']['CpcMax'] - kwDataUI['perTA']['CpcMax']);

  if (slope >= 0 ) {
    cpcMaxPlus1 = kwDataUI['perT']['CpcMax'] + 0.1;
  }
  else {
    cpcMaxPlus1 = kwDataUI['perT']['CpcMax'] - 0.1;
  }
